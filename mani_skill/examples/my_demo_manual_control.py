...
def main():
    ...
    while True:
        ...
        # -------------------------------------------------------------------------- #
        if args.enable_sapien_viewer:
            print("已选择enable_sapien_viewer，使用human模式进行渲染")
            env.render_human()

        # render_frame = env.render().cpu().numpy()[0]    #使用human渲染模式时该行报错AttributeError: 'Viewer' object has no attribute 'cpu'

        #修改渲染代码(添加106-115)，对于human渲染模式，从viewer对象中获取图像；对其他(如默认的RGB)渲染模式，获取pytorch张量
        if args.render_mode == "human":
            sapien_viewer = env.viewer
            print("渲染出sapien_viewer_frame")
            render_frame = sapien_viewer.render()
        else:
            render_frame = env.render()
            if hasattr(render_frame, 'cpu'):
                render_frame = render_frame.cpu().numpy()[0]
            else:
        ...
        # -------------------------------------------------------------------------- #
        # Input
        renderer(render_frame)  #使用human渲染模式时该行报错TypeError: Image data of dtype object cannot be converted to float
