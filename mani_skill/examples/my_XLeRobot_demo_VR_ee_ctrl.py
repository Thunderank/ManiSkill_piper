...
def get_mapped_joints(robot):
    ...
    if len(full_joints) >= 15:
        ...
        mapped_joints[3] = full_joints[6]   #ik中控制的关节1,代表机械臂连接底座的关节
        mapped_joints[4] = full_joints[9]   #ik中控制的关节2,代表机械臂中靠近夹爪腕部的前一个关节
...
def main(args: Args):
    ...
    oculus_reader = OculusReader(print_positions=False) #建立oculus reader对象
    ...
    current_joints = get_mapped_joints(robot)   #仍然用get_mapped_joints函数读取当前关节位置
    
    # Ensure target_joints is a numpy array with the same shape as current_joints
    target_joints = np.zeros_like(current_joints)
    
    # Set initial joint positions based on inverse kinematics from initial end effector positions   利用ik计算初始目标关节位置.只控制两个关节
    try:
    ...
    while True:
        ...
        # 添加夹爪控制变量
        gripper_force_arm1 = 0.0  # 0.0 = 完全打开, 1.0 = 完全闭合
        ...
        if step_counter >= warmup_steps:
            ...
            if 'l' in transforms:
                ...
                # 添加使用控制器旋转控制末端执行器姿态
                # pitch角控制夹爪俯仰
                pitch_2 = left_euler[1] * rotation_scale
            ...
            if 'r' in transforms:
                ...
                # 使用控制器旋转控制末端执行器姿态
                # pitch角控制夹爪俯仰
                pitch_1 = right_euler[1] * rotation_scale            
