"""
迁移自XLeRobot的VR控制示例，适配Piper机器人.但由于XLeRobot将其简化为2关节自由度,从而使用简单的ik,而只操作末端执行器,导致该控制方式无法令机械臂向前伸展
"""
...
"""
piper机械臂关节操作效果:
full_joints[0]  左右旋转 yaw    -2.65 ~ 2.65 逆时针/顺时针 
full_joints[1]  上下        0 ~ 1     平/下
full_joints[2]  小臂打开     -3.6 ~ 0        后伸展/平
4,6:手腕旋转
5:小臂上下摆
...
def main(args: Args):
    ...
    while True:
        ...
        if step_counter >= warmup_steps:
            ...
            if 'r' in transforms:
                ...
                if abs(x_new) > 0.05 or abs(z_new) > 0.05:
                    ...
                    target_joints[0] = rotation_angle * 1.2  # 基座旋转
        ...
        try:
            compensated_y = ee_pos[1] + tip_length * math.sin(pitch)
            target_joints[1], target_joints[2] = inverse_kinematics(ee_pos[0], compensated_y)
            target_joints[3] = pitch  # 俯仰角
        except Exception as e:
            print(f"计算逆运动学出错: {e}")
