'''
python -m mani_skill.examples.mycodes.cobot_pick_and_place_demo_simultaneously --render-mode="human" --shader="rt-fast" --reward_mode="dense" -c "pd_joint_delta_pos_dual_arm" -e "CobotPickAndPlaceCube-v1" -r "dual_piper"
'''
...
def cobot_pick_and_place_demo(env):
    ...
    #同时控制机械臂依循轨迹运动
    execute_trajectory(left_traj, right_traj)
...
#调用关系：
# execute_trajectory -> _move_arm_to_pose -> _follow_path -> _set_gripper

def execute_trajectory(left_traj, right_traj):
    assert len(left_traj) == len(right_traj), "给出的左右臂轨迹数量不一致"
    for step in range(len(left_traj)):
        print(f"执行第{step+1}/{len(left_traj)}步：")
        current_qpos = robot.get_qpos()[0].numpy().squeeze()    # 获取当前完整关节状态

        #读取左臂轨迹信息
        left_arm_pose_world = np.concatenate([left_traj[step]["xyz"], left_traj[step]["quat"]]).tolist()
        left_arm_base_link = robot.find_link_by_name("left_arm_base_link")

        #读取右臂轨迹信息
        right_arm_pose_world = np.concatenate([right_traj[step]["xyz"], right_traj[step]["quat"]]).tolist()
        right_arm_base_link = robot.find_link_by_name("right_arm_base_link")

        #分别进行mplib路径规划
        left_arm_result = plan_arm("left_arm", left_arm_pose_world, left_arm_base_link, current_qpos)
        right_arm_result = plan_arm("right_arm", right_arm_pose_world, right_arm_base_link, current_qpos)

        _follow_path(left_arm_result, right_arm_result)
        _set_gripper("left_arm", "open" if left_traj[step]["gripper"] else "close")
        _set_gripper("right_arm", "open" if right_traj[step]["gripper"] else "close")

        print("轨迹点执行完成\n")

def plan_arm(this_arm_name, this_pose_world, this_base_link, current_qpos):
    this_arm_joint_indices = arm_joint_indices[this_arm_name]   #相关joint index的List
    this_planner = planners[0] if this_arm_name == "left_arm" else planners[1]
    this_current_qpos = current_qpos[this_arm_joint_indices]    #提取相关joint的qpos
    assert len(this_current_qpos) == this_planner.joint_limits.shape[0], f"关节数量不匹配! 规划器期望 {this_planner.joint_limits.shape[0]} 个关节, 实际有 {len(this_current_qpos)} 个关节"
    
    this_arm_pose_robot = world_to_robot(this_pose_world, this_base_link.pose.raw_pose[0].tolist()) #获取当前机械臂root位姿，从而将目标点坐标转换到机械臂坐标系
    this_arm_result = this_planner.plan_screw(
        mplib.Pose(this_arm_pose_robot[:3], this_arm_pose_robot[3:]),           #必须使用机器人坐标系的值！必须转化为pose对象！
        this_current_qpos,                                                  # 使用当前手臂关节状态。必须是最新的结果
        time_step=1/140
    )
    if this_arm_result["status"] != "Success":
        print(f"{this_arm_name} 机械臂 screw规划失败，尝试RRTConnect")
        this_arm_result = this_planner.plan_pose(
            mplib.Pose(this_arm_pose_robot[:3], this_arm_pose_robot[3:]), 
            this_current_qpos,
            time_step=1/140
        )
        if this_arm_result["status"] != "Success":
            print(f"{this_arm_name} 机械臂 RRTConnect规划失败: {this_arm_result['status']}\n")
            return -1
        else:
            print(f"==================={this_arm_name} 机械臂 RRTConnect规划成功!===================")
    else:
        print(f"==================={this_arm_name} 机械臂 screw规划成功!===================")
    
    return this_arm_result


# 控制左右机械臂进行同时遵循轨迹，先执行完的机械臂会等待后执行完的机械臂
def _follow_path(left_arm_result, right_arm_result):
    # 获取左右臂的轨迹步数
    n_step_left = left_arm_result["position"].shape[0]
    n_step_right = right_arm_result["position"].shape[0]
    max_steps = max(n_step_left, n_step_right)  # 取最大步数
    ...
    for i in range(max_steps):
        ...
        if i < n_step_left:
            # 设置左臂关节
            left_arm_joints_index = arm_joint_indices["left_arm"]    
            for j in range(len(left_arm_joints_index) - 2):             # 去除两个夹爪joint
                joint = robot.get_active_joints()[left_arm_joints_index[j]]
                joint.set_drive_target(left_arm_result["position"][i][j])
                joint.set_drive_velocity_target(left_arm_result["velocity"][i][j])

        if i < n_step_right:
            # 设置右臂关节
            right_arm_joints_index = arm_joint_indices["right_arm"]    
            for j in range(len(right_arm_joints_index) - 2):             # 去除两个夹爪joint
                joint = robot.get_active_joints()[right_arm_joints_index[j]]
                joint.set_drive_target(right_arm_result["position"][i][j])
                joint.set_drive_velocity_target(right_arm_result["velocity"][i][j])
...
def _set_gripper(arm_name, state):
    arm_active_joints = [j for j in robot.get_active_joints() if j.get_name().startswith(f"{arm_name}_joint")]       #设置关节角时根据关节名称检索，与arm相关的所有joint
...
def demo():
    ...
    # 获取左右臂关节索引
    arm_joint_indices = {
        "left_arm": [idx for name, idx in joint_index_map.items() if "left_arm_joint" in name], #[8, 10, 12, 14, 16, 18, 20, 21]

        "right_arm": [idx for name, idx in joint_index_map.items() if "right_arm_joint" in name] #[9, 11, 13, 15, 17, 19, 22, 23]
    }
